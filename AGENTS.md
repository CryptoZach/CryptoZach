...
Every session (Cursor, Codex CLI, Claude Code, ad-hoc scripts) declares its role and write-scope at session start via `/tmp/session_role_$$.txt` (or the persistent-parent-PID variant when each shell spawn is ephemeral). File format per `handoff/multi_session_git_hooks_setup.md` Section 4. Required for the pre-commit hook scope-check to pass. Canonical role names (BULK-EXECUTOR, CANONICAL-WRITER, SITE-EDITOR, AUDIT-REVIEWER, AUTHOR-DIRECT) per `handoff/agent_roles_and_living_state_plan_2026-04-23.md`.
