...
### Lane discipline (DEC-069 superseded by DEC-111; effective 2026-04-25)

Per DEC-111 (Cursor deprecation; effective 2026-04-25), Claude Code in the **CANONICAL-WRITER role** handles all `docs/*.md` writes per AGENT_ROSTER Section 1.2 plus Section 2 plus Section 5 mechanism selection (State Manager specialization within CANONICAL-WRITER per Section 1.6). Sessions in BULK-EXECUTOR or other non-CANONICAL-WRITER roles (the **execution engine** lane: data pipelines, manuscript surgery, econometric estimation, content production, build tooling) capture canonical-worthy findings via `.cursor/tasks/Living_File_Updates_*.md` memos (pull-based; enumerated at session start by `scripts/claude-code-sync.py` per the Pending memos section) or via dispatch handoffs (push-based; Pattern 30 per DEC-110). Original DEC-069 framing ("Cursor is the sole canonical writer for `docs/`") preserved as historical-of-record in this subsection's heading, in `docs/DECISION_LOG.md` DEC-069 entry, and in the "Cursor deprecation (per DEC-111; effective 2026-04-25)" subsection below.

The one carve-out (DEC-069 carve-out preserved across DEC-111 supersedure; reframed for the role-based regime): `docs/PROGRAM_STATE.md` may receive Session Changes blocks from non-CANONICAL-WRITER commits when those commits are part of a coherent narrative (per the R1.5 / R1.5.1 pattern), but only after the same commit has staged the substantive content that the Session Changes block narrates. Other `docs/` files (DECISION_LOG, KEY_FINDINGS, DATA_REGISTRY, ENTITY_PROFILES, CROSS_REFERENCE_MAP, OUTREACH_LOG, KNOWN_UNKNOWNS, EDITORIAL_STANDARDS, SITE_STATE, HYPOTHESES, FRAMEWORK_REGISTRY, ERROR_CORRECTION_LOG, GLOSSARY, KNOWLEDGE_ARCHITECTURE; plus AGENT_ROSTER, READING_LIST, VERSION_HISTORY added in later cycles) are CANONICAL-WRITER-only writes per role per DEC-111.
...
- `docs/`: research state (18 canonical Markdown files; AGENT_ROSTER is the 18th per SM-2 canonicalization commit `2d91c56`; Claude Code CANONICAL-WRITER lane per DEC-111, supersedes DEC-069 Cursor lane assignment).
