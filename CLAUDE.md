...
Every Claude Code session declares its role and write-scope at session start via `/tmp/session_role_$$.txt` (or `/tmp/session_role_<CLAUDE_PID>.txt` when running out of Claude Code where each Bash tool call spawns a fresh shell). The file format is per `handoff/multi_session_git_hooks_setup.md` Section 4. Required for the pre-commit hook scope-check to pass; sessions that skip this step hit a hard-fail rejection on first commit. Canonical format and the five role names (BULK-EXECUTOR, CANONICAL-WRITER, SITE-EDITOR, AUDIT-REVIEWER, AUTHOR-DIRECT) per the synthesized plan `handoff/agent_roles_and_living_state_plan_2026-04-23.md`.
...
- **Logging discipline:** every commit during the observation period that involves Cursor and/or Claude Code MUST add a row to one of the two tables in `handoff/cursor_phased_reduction_observations.md` (same commit OR follow-up commit within 24 hours; whichever session ships the original work owns the ledger update). Cycles purely author-manual or pure-Cowork-equivalent do not require rows. Without enforced logging discipline, the ledger goes stale and exit-gate evaluation reverts to anecdote-driven (defeating Enhancement #1's denominator-providing purpose; per author 2026-04-23 evaluation [LOW-MEDIUM] gap addendum).
